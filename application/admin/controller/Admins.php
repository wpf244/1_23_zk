...
class Admins extends BaseAdmin
{
    public  function index(){
        ...
        if(empty($category)){
            ...
            $map['b.categoryid']=array("in",$arr);
        }
        
        //全部文章数量
        $qcou=db("article_info")->alias("a")->where($map)->where(["arttype"=>"xinwen"])->join("article_category b","a.id=b.articleid")->count();
        $this->assign("qcou",$qcou);

        

        //已审核
        $ycou=db("article_info")->alias("a")->where($map)->where(["arttype"=>"xinwen","reviewstatus"=>1])->join("article_category b","a.id=b.articleid")->count();
        ...
        //待审核
        $dcou=db("article_info")->alias("a")->where($map)->where(["arttype"=>"xinwen","reviewstatus"=>0])->join("article_category b","a.id=b.articleid")->count();
        ...
        //被退回
        $bcou=db("article_info")->alias("a")->where($map)->where(["arttype"=>"xinwen","reviewstatus"=>2])->join("article_category b","a.id=b.articleid")->count();
    ...
    public function get_cou($map)
    {
        ...
        $data[] = db("article_info")->alias("a")->where($map)->whereTime("createtime",'between',["$time6","$time6"."23:59:59"])->join("article_category b","a.id=b.articleid")->count();
        $data[] = db("article_info")->alias("a")->where($map)->whereTime("createtime",'between',["$time5","$time5"."23:59:59"])->join("article_category b","a.id=b.articleid")->count();
        $data[] = db("article_info")->alias("a")->where($map)->whereTime("createtime",'between',["$time4","$time4"."23:59:59"])->join("article_category b","a.id=b.articleid")->count();
        $data[] = db("article_info")->alias("a")->where($map)->whereTime("createtime",'between',["$time3","$time3"."23:59:59"])->join("article_category b","a.id=b.articleid")->count();
        $data[] = db("article_info")->alias("a")->where($map)->whereTime("createtime",'between',["$time2","$time2"."23:59:59"])->join("article_category b","a.id=b.articleid")->count();
        $data[] = db("article_info")->alias("a")->where($map)->whereTime("createtime","yesterday")->join("article_category b","a.id=b.articleid")->count();
        
        $data[] = db("article_info")->alias("a")->whereTime("createtime","d")->join("article_category b","a.id=b.articleid")->count();
