...
class News extends BaseAdmin
{
  ...
  //  var_dump($status);
    ...
    //  var_dump($where);exit;
       ...
       $list =db("article_info")->alias("a")->field("a.id,b_banner,title,author,createtime,top,reviewstatus,shenhename,name,articleid,b.categoryid")->where("arttype","xinwen")->where("b.categoryid","in",$arra)->where($where)->join("article_category b","a.id=b.articleid")->join("category_info c","c.id = b.categoryid")->group("b.articleid")->order(['top desc','id desc'])->paginate(20,false,['query'=>request()->param()]);
    ...
    public function lingdao()
    {
      ...
      foreach($data as $va){
       ...
       $list =db("article_info")->alias("a")->where("arttype","lingdao")->where("subtitle != '' ")->where("b.categoryid","in",$arra)->where($where)->join("article_category b","a.id=b.articleid")->join("category_info c","c.id = b.categoryid")->group("b.articleid")->order(['a.orderid desc','reviewstatus desc','a.id asc'])->paginate(20,false,['query'=>request()->param()]);
       $this->assign("list",$list);
    ...
    public function wenjian()
    {
      ...
      $arra=array();
       ...
       $list =db("article_info")->alias("a")->field("a.id,title,author,createtime,top,reviewstatus,shenhename,name,articleid,b.categoryid")->where("arttype","wenjian")->where("b.categoryid","in",$arra)->where("zhongguogovid = 0")->where($where)->join("article_category b","a.id=b.articleid")->join("category_info c","c.id = b.categoryid")->order(['top desc','id desc'])->paginate(20,false,['query'=>request()->param()]);
       $this->assign("list",$list);
